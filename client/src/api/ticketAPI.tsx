  ...
  try {
    ...
    if (!response.ok) {
      const errorMessage = contentType?.includes('application/json') 
        ? await response.json() 
        : await response.text(); // Handle non-JSON error messages
...
const retrieveTicket = async (id: number | null): Promise<TicketData> => {
  try {
    ...
    return contentType?.includes('application/json') ? await response.json() : Promise.reject('Invalid response format');
  } catch (err) {
    console.error('Error from data retrieval:', err);
